...
```bash
pip install yui-agent-policy==0.1.18
```
...
To verify the GitHub provenance for downloaded `0.1.18` artifacts, check the
...
PY
gh attestation verify "$verify_dir/yui_agent_policy-0.1.18-py3-none-any.whl" \
...
gh attestation verify "$verify_dir/yui_agent_policy-0.1.18.tar.gz" \
  --repo yui-stingray/agent-policy \
  --signer-workflow yui-stingray/agent-policy/.github/workflows/release.yml \
  --source-ref refs/tags/v0.1.18
)
