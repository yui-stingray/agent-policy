"""Subprocess contract tests for the fail-closed Claude Code hook."""
...
pytestmark = pytest.mark.skipif(BASH is None, reason="claude_code_hook.sh requires bash")


def _payload(tool_name: str, tool_input: dict[str, object], **extra: object) -> str:
...
def test_read_tool_is_allowed_silently() -> None:
    result = _run_hook(_payload("Read", {"file_path": "/tmp/x"}))
...
def test_bash_policy_block_has_fixed_sanitized_stderr() -> None:
    result = _run_hook(_payload("Bash", {"command": "git push --force origin main"}))
...
def test_safe_bash_forms_follow_auto_allow_policy(
    command: str,
    tmp_path: Path,
) -> None:
    result = _run_hook(
        _payload("Bash", {"command": command}),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        " \t\n",
        "[]",
        _payload("Read", {"file_path": "safe.txt"})
        + "\n"
        + _payload("Read", {"file_path": "safe.txt"}),
    ],
)
...
def test_inherited_xtrace_does_not_leak_protected_values(tmp_path: Path) -> None:
    ...
    result = _run_hook(
        _payload("Bash", {"command": f"printf %s {marker}-command"}),
        policy=policy,
        repo=f"{marker}-repo",
        xtrace=True,
    )
...
def test_unknown_claude_tool_blocks_explicitly() -> None:
    result = _run_hook(_payload("NewTool", {"command": "safe"}))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_payload("Bash", {"command": "ls -la"}), None, "acme/app"),
        (_payload("Bash", {"command": "ls -la"}), POLICY_TOML, None),
        (_payload("Bash", {"command": "ls -la"}), Path("/untrusted/config/policy.toml"), "acme/app"),
        (_payload("Bash", {}), POLICY_TOML, "acme/app"),
    ],
)
...
def test_missing_jq_blocks_with_the_same_sanitized_message(tmp_path: Path) -> None:
    result = _run_hook(_payload("Read", {"file_path": "/tmp/x"}), path=str(tmp_path))
...
def test_unknown_classifier_result_blocks(command: str) -> None:
    result = _run_hook(_payload("Bash", {"command": command}))
...
def test_external_auto_allow_write_requires_wrapper_owned_first_write_state(
    tmp_path: Path,
    first_write: str | None,
    expected_code: int,
) -> None:
    result = _run_hook(
        _payload("Write", {"file_path": "safe.txt"}),
...
@pytest.mark.parametrize("first_write", [None, "malformed"])
def test_external_read_only_tools_do_not_require_first_write_state(
    tmp_path: Path,
    first_write: str | None,
) -> None:
    result = _run_hook(
...
def test_external_first_write_comes_from_environment_not_payload(tmp_path: Path) -> None:
    result = _run_hook(
        _payload(
            "Write",
            {"file_path": "safe.txt", "first_write_to_repo": False},
            ownership_class="internal",
        ),
        policy=_external_auto_allow_policy(tmp_path),
        repo="someone-else/their-repo",
